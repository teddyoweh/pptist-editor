<template>
  <MoveablePanel 
    ...
    :width="300" 
    :height="560" 
    :title="`Comments for Slide ${slideIndex + 1}`" 
    :left="-270" 
    :top="90"
    :minWidth="300"
    :minHeight="400"
  ...
  >
    <div class="container">
      <div class="notes" ref="notesRef">
        ...
        <div class="empty" v-if="!notes.length">No comments on this page</div>
      </div>
      <div class="send">
        <TextArea 
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`Enter comment (for ${handleElementId ? 'selected element' : 'current slide'})`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <i-icon-park-outline:delete class="btn icon" v-tooltip="'Clear all comments on this page'" style="flex: 1" @click="clear()" />
