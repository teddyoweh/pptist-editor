...
// Current selection border line positions
...
// Current selection resize handle position
...
// Initialize chart data: format and fill into DOM
const initData = () => {
...
// Keyboard listener: Enter moves focus to next row
const moveNextRow = () => {
...
// Get current chart data from DOM, format and emit
const getTableData = () => {
  ...
  // First row is series names, first column is category names, actual data starts from row 2, column 2
  ...
  // Scatter chart must have exactly two columns of data
...
const changeSelectRange = (e: MouseEvent) => {
  ...
  document.onmouseup = e => {
    ...
    // When drag ends, if range exceeds half a cell, auto-expand to next cell (e.g., dragging to 1.5 cells will expand to 2 cells, applies both horizontally and vertically)
