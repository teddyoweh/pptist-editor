...
// When component renders, clear element focus if it exists
// This happens when: entering presentation mode with focused elements, then exiting (page may have changed)
onMounted(() => {
...
// Click blank canvas area: clear focused elements, set canvas focus, clear text selection, clear format painter
...
const handleDblClick = (e: MouseEvent) => {
  ...
  createTextElement({
    left,
    top,
    width: 200 / canvasScale.value, // Divide by canvasScale to maintain consistent width with click-selection creation
    height: 0,
  })
}

// Clear format painter state when canvas unmounts
onUnmounted(() => {
  if (textFormatPainter.value) mainStore.setTextFormatPainter(null)
})
