...
export default () => {
  ...
  const uniformHorizontalDisplay = () => {
    ...
    // Get individual elements and grouped elements separately, recording the range of each item
    ...
    // Combine individual elements and grouped elements, then sort by position (left to right)
    ...
    // (total range of selected elements - sum of all selected element widths) / (number of selected elements - 1)
    let totalWidth = 0
    ...
    // Calculate the target position for each element in order
    // The first item is the starting point, no calculation needed
    // From the second item onwards, each position should be: previous position + previous width + spacing
    // Note: the position (pos) calculated here is not the element's final left value, but the minimum value of the target position range (element's left value after rotation ≠ range minimum)
    ...
    // Calculate the element's final target left value based on the target position
  ...
  // Distribute elements evenly vertically (logic similar to horizontal distribution method)
